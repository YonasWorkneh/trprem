  ...
  const fetchSends = async () => {
    try {
      ...
      const { data: withdrawalsData, error: withdrawalsError } =
        await supabase.rpc("get_admin_withdrawals", { type_filter: "send" });

      if (withdrawalsError) throw withdrawalsError;

      if (withdrawalsData && withdrawalsData.length > 0) {
        const userIds = [
          ...new Set(withdrawalsData.map((w: any) => w.user_id)),
        ];
      ...
      <CardHeader className="border-b border-border/50 bg-muted/20">
        <CardTitle className="flex items-center gap-2">
          ...
          <Badge variant="secondary" className="ml-2">
            {sends.filter((s) => s.status === "pending").length} Pending
          </Badge>
        </CardTitle>
      </CardHeader>
              ...
              {sends.length === 0 && (
                <TableRow>
                  <TableCell colSpan={6} className="p-0">
                    <AdminEmptyState
                      title="No send requests found"
                      description="When users send funds to other wallets, they will appear here for your review."
                      icon={Send}
                    />
                  </TableCell>
                </TableRow>
